...
public class ServerHandlerCtrl {
	...
	public boolean isAliasAvailable(String alias) {
		...
		try {
			Form searchForm = search.getSearchForm(AardvarkApp.getContext().getString(edu.chalmers.aardvark.R.string.server_search_address));
			Form answerForm = searchForm.createAnswerForm();
			answerForm.setAnswer("alias", alias);
			aliasData = search.getSearchResults(answerForm, AardvarkApp.getContext().getString(edu.chalmers.aardvark.R.string.server_search_address));
		} catch (XMPPException e) {
	...
	public void logInWithAlias(String alias) {
		...
		ServerConnection.login(LocalUser.getLocalUser().getAardvarkID(), LocalUser.getPassword());
